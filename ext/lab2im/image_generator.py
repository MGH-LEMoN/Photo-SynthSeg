...
# project imports
from . import edit_volumes, utils
    ...
    def _build_model_inputs(self, n_labels):
        ...
        while True:
            ...
            for idx in indices:
                ...
                y = utils.load_volume(self.labels_paths[idx],
                                      dtype='int',
                                      aff_ref=np.eye(4))
                ...
                for channel in range(self.n_channels):
                    ...
                    if isinstance(self.prior_means, np.ndarray):
                        if (self.prior_means.shape[0] >
                                2) & self.use_specific_stats_for_channel:
                            ...
                            tmp_prior_means = self.prior_means[2 * channel:2 *
                                                               channel + 2, :]
                        else:
                    ...
                    if isinstance(self.prior_stds, np.ndarray):
                        if (self.prior_stds.shape[0] >
                            ...
                            tmp_prior_stds = self.prior_stds[2 * channel:2 *
                                                             channel + 2, :]
                        else:
                    ...
                    tmp_means = utils.add_axis(
                        tmp_classes_means[self.generation_classes],
                        axis=[0, -1])
                    tmp_stds = utils.add_axis(
                        tmp_classes_stds[self.generation_classes],
                        axis=[0, -1])
