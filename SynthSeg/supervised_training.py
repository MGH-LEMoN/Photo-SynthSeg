...
def supervised_training(
    ...
    input_generator = utils.build_training_generator(
        build_model_inputs(path_images, path_labels, batchsize), batchsize
    )
...
def build_augmentation_model(
    ...
    if data_res is not None:
        ...
        downsample = (
            utils.reformat_to_list(downsample, n_channels)
            if downsample
            else np.min(thickness - data_res, 1) < 0
        )
        target_res = (
            atlas_res
    ...
    image = layers.IntensityAugmentation(
        6, clip=False, normalise=True, gamma_std=0.4, separate_channels=True
    )(image)
