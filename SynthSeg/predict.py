...
def predict(
    ...
    if (n_neutral_labels is not None) & flip:
        ...
        lr_corresp = np.stack(
            [
                segmentation_labels[
                    n_neutral_labels : n_neutral_labels + n_sided_labels
                ],
    ...
    for idx, (
        ...
        zip(path_images, path_segmentations, path_posteriors, path_resampled, compute)
    ):
        ...
        if tmp_compute:
            ...
            (
                ...
                path_image, n_levels, target_res, cropping, padding, flip, path_resample
            )
...
def prepare_output_files(
    ...
    if (
        ...
        if out_posteriors is not None:
            if (
                (out_posteriors[-7:] == ".nii.gz")
                | (out_posteriors[-4:] == ".nii")
                | (out_posteriors[-4:] == ".mgz")
                | (out_posteriors[-4:] == ".npz")
            ):
                raise Exception(
                    "Output folders cannot have extensions: "
    ...
    # path_images is an image
        ...
        if out_posteriors is not None:
            if (
                (".nii.gz" not in out_posteriors)
                & (".nii" not in out_posteriors)
                & (".mgz" not in out_posteriors)
        ...
        if out_resampled is not None:
            if (
                (".nii.gz" not in out_resampled)
                & (".nii" not in out_resampled)
                & (".mgz" not in out_resampled)
    ...
    recompute_list = [
        ...
        for (re_seg, re_post, re_res) in zip(
            recompute_seg, recompute_post, recompute_resampled
        )
    ]
...
def preprocess_image(
    ...
    if crop is not None:
        im, crop_idx = edit_volumes.crop_volume(
            im, cropping_shape=crop, return_crop_idx=True
        )
    else:
        crop_idx = None
    ...
    if flip & (n_dims > 2):
        ...
        im_flipped = (
            utils.add_axis(im_flipped)
            if n_channels > 1
            else utils.add_axis(im_flipped, axis=[0, -1])
        )
    else:
        im_flipped = None
...
def postprocess(
    ...
    if post_patch_flip is not None:
        post_patch_flip = edit_volumes.flip_volume(
            np.squeeze(post_patch_flip), direction="rl", aff=np.eye(4)
        )
    ...
    if im_shape != pad_shape:
        ...
        posteriors = edit_volumes.crop_volume_with_idx(
            posteriors, bounds, n_dims=n_dims
        )

    # align prediction back to first orientation
    if n_dims > 2:
        seg = edit_volumes.align_volume_to_ref(
            seg, aff=np.eye(4), aff_ref=aff, n_dims=n_dims, return_aff=False
        )
        posteriors = edit_volumes.align_volume_to_ref(
            posteriors, aff=np.eye(4), aff_ref=aff, n_dims=n_dims
        )
