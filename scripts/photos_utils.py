...
def nonlinear_deformation():
    ...
    small_shape = l2m_utils.get_resample_shape(in_vol.shape, nonlin_shape_factor)
...
def create_symlink(source, num_copies=10):
    ...
    list(
        map(
            lambda x: os.symlink(source, create_destination_name(source, x)),
            range(1, num_copies),
        )
    )
...
def find_label_differences():
    ...
    file_list = [
        file
        for file in sorted(glob.glob(os.path.join(config.LABEL_MAPS_DIR, "*.nii.gz")))
        if "copy" not in file
    ]
...
def get_recon_shapes():
    ...
    for folder in folder_list:
        ...
        file3 = os.path.join(folder, "ref_mask", "manual_labels_merged.elastix.mgz")
...
def model_dice_map():
    ...
    dice_list = []
    for model_dir in model_dirs:
        last_dice_file = sorted(glob.glob(os.path.join(model_dir, "dice_*")))[-1]
...
def pipeline3():
    ...
    os.makedirs(
        "/space/calico/1/users/Harsha/SynthSeg/trash/20220408/mri.masked", exist_ok=True
    )
