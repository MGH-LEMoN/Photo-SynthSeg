...
samseg_gather_dict = {
    "hard_samseg": {
        "source": "SAMSEG_OUTPUT_HARD",
        "destination": "HARD_SAMSEG_NEW",
        "expr": ["seg.mgz"],
        "message": "Hard SAMSEG",
    },
    "soft_samseg": {
        "source": "SAMSEG_OUTPUT_SOFT",
        "destination": "SOFT_SAMSEG_NEW",
        "expr": ["seg.mgz"],
        "message": "Soft SAMSEG",
    },
}
...
def write_correlations_to_file(config, suffix=None):
    ...
    hard_samseg_vols = extract_samseg_volumes(config,
                                              config.HARD_SAMSEG_STATS_NEW,
                                              "hard")
    soft_samseg_vols = extract_samseg_volumes(config,
                                              config.SOFT_SAMSEG_STATS_NEW,
                                              "soft")
...
class Configuration:
    def __init__(self, project_dir, project_suffix, out_folder):
        ...
        self.SAMSEG_OUTPUT_HARD = f"{self.SYNTHSEG_RESULTS}/SAMSEG_OUTPUT_HARD"
        self.SAMSEG_OUTPUT_SOFT = f"{self.SYNTHSEG_RESULTS}/SAMSEG_OUTPUT_SOFT"
        ...
        self.HARD_REF_WARPED = f"{self.SYNTHSEG_RESULTS}/UW.hard.warped.ref"
        self.HARD_RECONS = f"{self.SYNTHSEG_RESULTS}/UW.hard.recon"
        self.HARD_SYNTHSEG = f"{self.SYNTHSEG_RESULTS}/UW.hard.synthseg"
        self.HARD_SAMSEG = f"{self.SYNTHSEG_RESULTS}/UW.hard.samseg"
        self.HARD_SAMSEG_NEW = f"{self.SYNTHSEG_RESULTS}/UW.hard.samseg.new"
        ...
        self.SOFT_REF_WARPED = f"{self.SYNTHSEG_RESULTS}/UW.soft.warped.ref"
        self.SOFT_SYNTHSEG = f"{self.SYNTHSEG_RESULTS}/UW.soft.synthseg"
        self.SOFT_SAMSEG = f"{self.SYNTHSEG_RESULTS}/UW.soft.samseg"
        self.SOFT_SAMSEG_NEW = f"{self.SYNTHSEG_RESULTS}/UW.soft.samseg.new"
        ...
        self.HARD_SAMSEG_STATS_NEW = f"{self.SAMSEG_OUTPUT_HARD}/"
        self.SOFT_SAMSEG_STATS_NEW = f"{self.SAMSEG_OUTPUT_SOFT}/"
...
if __name__ == "__main__":
    ...
    new_dice2d_dict = []
    copy_relevant_files(config, samseg_gather_dict)
    write_correlations_to_file(config, suffix="new")
    move_volumes_into_target_spaces(config, mri_convert_items1)
    calculate_dice_for_dict(config, new_dice2d_dict)
    write_plots(config)
