...
def dice_plot_from_df(config, df, out_file_name, flag):
    ...
    ax.set_xticklabels(
        config.LABEL_PAIR_NAMES, rotation=45, color="k", fontweight="bold", ha="right"
    )
    ...
    ax.legend(
        handles=handles, labels=labels, fontsize=20, frameon=True, edgecolor="black"
    )

    plt.savefig(
        os.path.join(
            config.SYNTHSEG_RESULTS, "images", config.model_name + "_" + out_file_name
        )
    )


def construct_dice_plots_from_files(
    config, file1, file2, merge_flag, hard_or_soft, out_name
):
...
def extract_scores(config, in_file_name, merge=0):
    ...
    hard_dice_json = os.path.join(config.SYNTHSEG_RESULTS, "dice_files", in_file_name)
...
def create_single_dataframe(config, data1, data2):
    ...
    ha2 = ha2.rename(columns=dict(zip(ha2.columns, ["struct", "subject", "score"])))
    ha2["type"] = "synthseg"
